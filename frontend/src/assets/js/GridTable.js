...
class GridTable {
	...
	constructor(rootElement, params) {
		...
		this.visibleIndices = this.filteredIndex;
		this.nodeOffsetY = 0;
	...
	prepareVirtualScroll(scrollTop, reset){
		...
		this.visibleIndices = this.filteredIndex.slice(this.startNode, this.startNode + this.visibleNodesCount);
	...
	alterContent(keepCurrent){
		if(this.sorted){
			this.visibleIndices.map((index, rowNumber) => this.getRowDataAt(index, rowNumber))
	...
	toggleDropdown(e){
		...
		const rect = {
			...
			minLeft: this.sizeBase.widths[0],
...
class Dropdown{
	...
	open(index, values, rect){
		...
		if(this.filtered && this.currentColumnIndex == this.filteredColumn){
			...
			this.filteredIndex = this.values.map((e,i) => i);
	...
	_prepareVirtualScroll(scrollTop, reset){
		...
		this.visibleIndices = this.filteredIndex.slice(this.startNode, this.startNode + visibleNodesCount);
